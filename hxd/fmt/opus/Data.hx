...
class Data extends hxd.snd.Data {
	...
	override function decodeBuffer(out:haxe.io.Bytes, outPos:Int, sampleStart:Int, sampleCount:Int) {
		if (currentSample != sampleStart) {
			...
			if (!opus_seek(reader, sampleStart))
	...
	@:hlNative("opus", "opus_open") static function opus_open(bytes:hl.Bytes, size:Int):OpusFile {
		return null;
	}

	@:hlNative("opus", "opus_seek") static function opus_seek(o:OpusFile, sample:Int):Bool {
		return false;
	}
