...
static int opus_memread(void *stream, unsigned char *ptr, int nbytes) {
	...
	memcpy(ptr, o->bytes + o->pos, (size_t)len);
	o->pos += len;
	return len;
}

static int opus_memseek(void *stream, opus_int64 offset, int whence) {
	fmt_opus *o = (fmt_opus *)stream;
	opus_int64 newpos;
	...
	switch (whence) {
		case SEEK_SET:
			...
			break;
		case SEEK_CUR:
			newpos = (opus_int64)o->pos + offset;
			break;
		case SEEK_END:
			newpos = (opus_int64)o->size + offset;
			break;
		default:
			return -1;
	}
...
static const OpusFileCallbacks OPUS_CALLBACKS_MEMORY = {
	opus_memread,
	opus_memseek,
	opus_memtell,
	NULL
...
HL_PRIM int HL_NAME(opus_read)(fmt_opus *o, char *output, int size, int format) {
	...
	while (samples_requested > 0) {
		...
		if (format == 2)
			ret = op_read(o->f, (opus_int16 *)output, samples_requested * channels, NULL);
		else
			ret = op_read_float(o->f, (float *)output, samples_requested * channels, NULL);
		...
		bytes_read = ret * channels * bytes_per_sample;
		total += bytes_read;
		output += bytes_read;
